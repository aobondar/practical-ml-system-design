...
Let's assume that you're working on a chatbot system to automate customer support for a large organization. Your system optimizes time of support stuff by automation, but gives worse user experience, that a trained personel. So you want to increase automation, without sarificing quality too much. Technically your architecture has intent classifier, which uses NLP to understand user's problem, and set of designed procedures, connected to intents, which guides users throught problem solving flow. There are metrics you can use to track your progress, while working on intent classification problem.
...
Even if your key metric is any other but event based metric, it's useful to have at least one straightforward event based metric. It helps you in cases where a composite/modelled metric shows weird results and you want to doublecheck yourself "is this a real effect, or a metric's artifact". For example: you're making a recommender system, which should improve forecased LTV. You're pretty sure, that you've made a positive change in your system, but your A/B on LTV is red. In that case if the CTR is green it makes sense to try to figure out first, is your LTV is forecasted well enough, before trashing your experiment.

### 1.3.2 Annotation based

Annotation based metrics are pretty common in NLP tasks. It usually happens, when:
* A model is a part of biger pipeline, so you want to track quality of NLP model in isolation (like in chatbot scenario you may want to track the quality of intent classification apart from other business logic around it)
...
* **Lack of domain knowledge or motivation**. If your annotation workers have no expirience in the field or not really interested in doing good job - in that case your metric will be pretty unreliable even with the best instruction.
...
* **Check agreement rate between annotators**. Now you can try the same process with annotators and bigger dataset. If your overall agreement is low - then it doesn't make any sense to track this metric at all (without adjustments). We will discuss later particular criteries of good and bad agreement rates.

* **Mix honeypots (gold standards) into annotation tasks to aproximately calculate metric relibility** If you see, that your annotators understand the task, you still want to estimeate the metric's noise. To do this you can make a golden set of annotations
