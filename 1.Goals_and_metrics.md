![image](https://github.com/user-attachments/assets/8ad645cd-84b8-40b7-8c3c-af1a4be9b2a5)# 1. Goals and metrics
...
|                                  | Diff in operator's time per customer for groups with and without the bot | 🔥Great (excellent if all operators are paid equally)               | ⚠️OK (via A/B testing, though it might be affected by behavior differences in users with/without the bot, affected by different conversion rates on different intents) | ⚠️OK (some people receive a degraded service, data collection takes time) | 🔥Great     | 🔥Yes             |
|                                  | Number of fully automated dialogues                        | ✅Good (some dialogs are easier to automate than others)           | ⚠️OK (via A/B testing, though it might be affected by behavior differences in users with/without the bot, affected by different conversion rates on different intents) | ✅Good (automatically generated, but time for data collection is needed) | 🔥Great     | 🔥Yes             |
...
| | annotation on ramdom sample from requests stream "is problem solved" | 🔥Great | 🔥Great | ⚠️OK (requires manual annotation) | ⚠️OK | ❌No |
...
* Start with a set of metrics focusing on "automation" combined with "manual annotation on quality." This is cheap to set up and allows you to jumpstart the project while bringing business value.
* Conduct a simple A/B test comparing scenarios "with bot" versus "without bot" to check if your proxy metrics correlate with actual business metrics such as CSAT or cost savings. If the results are satisfactory, you can rely on these proxies for some time.
