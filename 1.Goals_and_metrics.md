...
As you can see, there is no ideal set of metrics for real-world applications. But usually you don't need one: at the early stages of a system, when your understanding of the domain is limited and the solution is still rough, any adequate metric will guide you correctly. Indeed, having the low-base effect with a baseline solution means that any changes will be significant enough to be detected even by coarse metrics. As the project matures, you can refine your metric system to make it more precise. For this particular case with the chatbot, your course of action could be:

* Agree with the stakeholders that your target metrics will be saved money and CSAT. However you will begin by working on proxy metrics without waiting for the full analytics framework on the key metrics to be constructed.
...
* **Check the metric's confidence interval** To do this take N random samples from production distribution, annotate it with trusted peersonell as a gold standard and then push these samples into your production annotation pipeline. You'll have estimated error rate of **n_mistakeen_annotations / N**. The rule of the thumb on sample size is: having 100 examples will give you error rate estimation with +/-3% accuracy. On 300 this variance goes down to 1% and on 1000 it goes to half a percent. And error rate is easy to apply to a metric: your real meetric value lays in *(between metric - error_rate, metric + error_rate)* interval. On your particular case you can analytically improve this estimation (by examining actual distribution of errors and taking into considirations all dependencies between variables).
...
* **General quality degradation**. Your customer's behaviour patterns can break because of global trends, and you can find it out way too late, when your real LTV is way lower, than forecasted. So lower your forecast period, lower the risk.

* **Positive feedback loops between models** Let's assume you're building marketing recommendations models to increase total expected LTV (exLTV). Naturally you focus your campaign on high-LTV cohort. Your LTV models gets more data on high-LTV users. Your model narrows down high-exLTV cohort. Your recommendation model focuses on that more narrow cohort. And so on. To avoid this kind of cycles you want to establish independent exploration strategies for both porxy model and models, which targets that proxy.
