...
* **decreases amount of reports twice** online metric, to check your progress.
* **recall** > **0.5** (halve amout of non-detected content. Acceptable fluctuation here is 0.03)
* **positive_rate** < **0.001** (you can't process more than 15K of items, which is 0.1% of your stream. Acceptable fluctuation here is 0.0003 - bigger fluctuation could cause additional hiring for content team, which will be considered as failure)

Key hypothesis for this project is:
* We'll train binary classification model (biggset reasonable from economical standpoint) on historically annotated data (or additionally annotated if needed) and it'll give desired metrics staticly
* Dynamically our update process with parameters F (frequency - how often we re'train model) and T (T - historical time-window for training data) will keep metrics in desired region.

Let's consider risks:
1. Even having ideal model we won't be able to reduce reports by half. This can happen if there are much more abusive content, than reported, and you can't tell repored from non-reported just by content itself. In absurd scenario: if half of the content is inappropriate, and only 1% is reported, than you have to moderate half of created content (which is impossible in our setup).
2. With any historical window for data collection we won't achieve desired metrics in terms of model accuracy.
3. 
