...
2. **Problem is too complex for current generation of models** With any historical window for data collection we won't achieve desired metrics in terms of model accuracy.
3. **Problem is solvable, but going to be way too expensive**. It can be solved with LLM, but it'll be way too expensive.
4. **Problem is solvable for a static distribution, but will break up on distribution shifts**. Lack of generalization can cause spikes in false positives or false negatives, which will cause dropping down metrics and diminishing business value.
...
This mean, that we firstly want to take off the table the highest risks, and then evaluate more minor ones. Let's see how experiments cycle can look here.
