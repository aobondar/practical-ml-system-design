...
Let's assume your current moderation team does 10000 checks a day (and this number doesn't exceed amount of abusive content in a system). In successfull scenario 5K checks are dedicated to user-reported materials. and 15K can be occupied by automatic moderation system processes. Let's assume that your userbase generates 150'000 content pieces a day. In that case your goal is to implement a classifier (abusive / non abusive), which:

* **recall** > **0.5** (halve amout of non-detected content)
* **positive_rate** < **0.1** (you can't process more than 15K of items, which is 10% of your stream)

Business can digest fluctuation of these metrics for **3%%** (these fluctuations in volume can be covered by temporary overtime of existing personell).
