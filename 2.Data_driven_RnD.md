...
* Every single users' request is a unique problem, which can't be addressed by self-service non-presonalized instruction.
* Users wouldn't be able (or want to) find answer in FAQ and will directly go to the human operator (even when FAQ could solve their problem).
* We can't build efficient enough clustering process to cover enough user problems to achieve and keep **30%** of automation
* Users will have too low CSAT on self-service scenario
...
* Take **300** users' dialogs with customer support from production distribution (it will give around **1%%** of variance in your estimations)
* Manually annotated it for 3 classes: "Already have FAQ article for that case" | "We can write FAQ article for that case" | "Can't be solved with FAQ"

This cheap and simple trick will give you a glance on what are theoretical boundaries of your automation using this approach.
